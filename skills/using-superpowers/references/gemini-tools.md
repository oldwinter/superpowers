...
> **中文导读：** 本表把 skill 中的抽象动作映射到 Gemini CLI 工具。读取、编辑、shell、搜索、skill 调用、todo 和 subagent 分派都必须使用表内对应项；个人 skills 优先使用 `~/.gemini/skills/`，也识别跨 runtime 的 `~/.agents/skills/`。`Subagent (general-purpose)` 模板应填完全部 placeholders 后，通过 `invoke_agent` 的 `generalist` 执行。
